...
class sep0:
    ...
    # ----- Initialize ----- #
    def __init__(self, input_image, bw=32, bh=32, fw=3, fh=3,
                 thresh=3.0, minarea=5, db_nth=32, db_cont=0.005):
        self.img = input_image
        self.hdu = fits.open(input_image)[0]
        self.dat = self.hdu.data.byteswap().newbyteorder()
        self.hdr = self.hdu.header
        self.wcs = wcs.WCS(self.hdr)
        self.zmag = -2.5*np.log10(self.hdr['PHOTFLAM'])-5.0*np.log10(self.hdr['PHOTPLAM'])-2.408
        self.gain = self.hdr['CCDGAIN']
    ...
    def AutoPhot(self, Kron_fact=2.5, min_diameter=3.5, write=True):
        ...
        if write:
            df.to_csv(ip.tmp_dir+'auto_'+self.img.split('.fits')[0]+'.csv')

            f = open(ip.tmp_dir+'src_'+self.img.split('.fits')[0]+'.reg','w')
    ...
    def AperPhot(self, r_aper=[1.5, 2.0, 4.0], write=True):
        ...
        if write:
            df.to_csv(ip.tmp_dir+'aper_'+self.img.split('.fits')[0]+'.csv')
    ...
    def Pick_PointSrc(self, mag_high=20.0, mag_low=26.0, size_low=1.0, size_high=2.0, ar_cut=0.75, write=True):
        ...
        if write:
            f = open(ip.tmp_dir+'poi_'+self.img.split('.fits')[0]+'.reg','w')
